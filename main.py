...
class ClashPediaApp:
    def __init__(self):
        ...
        self.window.geometry('1800x980')
        self.window.minsize(1500, 900)
    ...
    def card_tile(self, parent, card, on_click=None, compact=False):
        ...
        frame = tk.Frame(
            ...
            width=190 if compact else 320,
            height=250 if compact else 420,
        )
        ...
        if image_path and Path(image_path).exists():
            size = (118, 165) if compact else (170, 235)
            ...
            placeholder = tk.Frame(frame, width=112 if compact else 140, height=138 if compact else 180, bg='#edf3f8', bd=0)
        ...
        tk.Label(
            ...
            wraplength=168 if compact else 280,
            justify='center',
        ).pack(fill=X, padx=8)
    ...
    def render_cards_page(self):
        ...
        outer = ScrolledFrame(self.content, autohide=False, padding=16)
        ...
        search_entry = ttk.Entry(search_row, textvariable=self.current_search, width=38)
        ...
        detail_box = tk.Frame(body, bg=SURFACE, bd=1, relief='solid', width=760)
        detail_box.pack(side=RIGHT, fill=Y)
        detail_box.pack_propagate(False)

        tk.Label(catalog_box, text='Card catalog', bg=SURFACE, fg=TEXT, font=('Segoe UI', 13, 'bold')).pack(anchor='w', padx=16, pady=(14, 4))
        tk.Label(catalog_box, text='Click any card to load the full details panel.', bg=SURFACE, fg=SUBTEXT, font=('Segoe UI', 9)).pack(anchor='w', padx=16, pady=(0, 10))

        catalog_scrollbar = ttk.Scrollbar(catalog_box, orient='vertical')
        catalog_scrollbar.pack(side=RIGHT, fill=Y, padx=(0, 6), pady=(0, 12))
        catalog_canvas = tk.Canvas(catalog_box, bg=SURFACE, highlightthickness=0, bd=0, yscrollcommand=catalog_scrollbar.set)
        catalog_canvas.pack(side=LEFT, fill=BOTH, expand=True, padx=(12, 0), pady=(0, 12))
        catalog_scrollbar.configure(command=catalog_canvas.yview)

        catalog_inner = tk.Frame(catalog_canvas, bg=SURFACE)
        catalog_window = catalog_canvas.create_window((0, 0), window=catalog_inner, anchor='nw')

        def sync_catalog_region(event=None):
            catalog_canvas.configure(scrollregion=catalog_canvas.bbox('all'))
            catalog_canvas.itemconfigure(catalog_window, width=catalog_canvas.winfo_width())

        catalog_inner.bind('<Configure>', sync_catalog_region)
        catalog_canvas.bind('<Configure>', lambda e: catalog_canvas.itemconfigure(catalog_window, width=e.width))
        def bind_catalog_wheel(_event=None):
            catalog_canvas.bind_all('<MouseWheel>', _on_mousewheel)
            catalog_canvas.bind_all('<Button-4>', _on_mousewheel_linux)
            catalog_canvas.bind_all('<Button-5>', _on_mousewheel_linux)

        def unbind_catalog_wheel(_event=None):
            catalog_canvas.unbind_all('<MouseWheel>')
            catalog_canvas.unbind_all('<Button-4>')
            catalog_canvas.unbind_all('<Button-5>')

        def _on_mousewheel(event):
            catalog_canvas.yview_scroll(int(-1 * (event.delta / 120)), 'units')

        def _on_mousewheel_linux(event):
            catalog_canvas.yview_scroll(-1 if getattr(event, 'num', None) == 4 else 1, 'units')

        catalog_canvas.bind('<Enter>', bind_catalog_wheel)
        catalog_canvas.bind('<Leave>', unbind_catalog_wheel)

        tk.Label(detail_box, text='Card details', bg=SURFACE, fg=TEXT, font=('Segoe UI', 15, 'bold')).pack(anchor='w', padx=16, pady=(16, 4))
        tk.Label(detail_box, text='The full card profile appears here when you click a card.', bg=SURFACE, fg=SUBTEXT, font=('Segoe UI', 9)).pack(anchor='w', padx=16, pady=(0, 10))

        detail_scrollbar = ttk.Scrollbar(detail_box, orient='vertical')
        detail_scrollbar.pack(side=RIGHT, fill=Y, padx=(0, 6), pady=(0, 16))
        detail_canvas = tk.Canvas(detail_box, bg=SURFACE, highlightthickness=0, bd=0, yscrollcommand=detail_scrollbar.set)
        detail_canvas.pack(side=LEFT, fill=BOTH, expand=True, padx=(12, 0), pady=(0, 16))
        detail_scrollbar.configure(command=detail_canvas.yview)

        detail_inner = tk.Frame(detail_canvas, bg=SURFACE)
        detail_window = detail_canvas.create_window((0, 0), window=detail_inner, anchor='nw')

        def sync_detail_region(event=None):
        ...
        detail_inner.bind('<Configure>', sync_detail_region)
        detail_canvas.bind('<Configure>', lambda e: detail_canvas.itemconfigure(detail_window, width=e.width))

        detail_title = tk.Label(detail_inner, text='Select a card', bg=SURFACE, fg=TEXT, font=('Segoe UI', 18, 'bold'))
        detail_title.pack(anchor='w', pady=(0, 8))

        detail_subtitle = tk.Label(detail_inner, text='Name, rarity, elixir, arena, property, description, and stats will appear below.', bg=SURFACE, fg=SUBTEXT, font=('Segoe UI', 10), wraplength=680, justify='left')
        detail_subtitle.pack(anchor='w', pady=(0, 12))

        detail_image_label = tk.Label(detail_inner, bg=SURFACE)
        detail_image_label.pack(anchor='center', pady=(0, 12))

        info_title = tk.Label(detail_inner, text='Overview', bg=SURFACE, fg=TEXT, font=('Segoe UI', 12, 'bold'))
        info_title.pack(anchor='w', pady=(4, 8))
        info_grid = tk.Frame(detail_inner, bg=SURFACE)
        info_grid.pack(fill=X, pady=(0, 12))

        desc_title = tk.Label(detail_inner, text='Description', bg=SURFACE, fg=TEXT, font=('Segoe UI', 12, 'bold'))
        desc_title.pack(anchor='w', pady=(6, 4))
        detail_desc = tk.Label(detail_inner, bg=SURFACE, fg='#334155', justify='left', wraplength=700, font=('Segoe UI', 10), anchor='w')
        detail_desc.pack(fill=X, anchor='w')

        stats_title = tk.Label(detail_inner, text='Stats', bg=SURFACE, fg=TEXT, font=('Segoe UI', 12, 'bold'))
        stats_title.pack(anchor='w', pady=(14, 8))
        stats_grid = tk.Frame(detail_inner, bg=SURFACE)
        stats_grid.pack(fill=X, pady=(0, 12))

        def clear_detail_frames():
            for child in info_grid.winfo_children():
                child.destroy()
            for child in stats_grid.winfo_children():
                child.destroy()

        def add_field(parent, label, value, row, col):
            box = tk.Frame(parent, bg='#f6f9fc', bd=1, relief='solid')
            box.grid(row=row, column=col, sticky='nsew', padx=5, pady=5)
            tk.Label(box, text=label, bg='#f6f9fc', fg=SUBTEXT, font=('Segoe UI', 8)).pack(anchor='w', padx=10, pady=(8, 0))
            tk.Label(box, text=value, bg='#f6f9fc', fg=TEXT, font=('Segoe UI', 10, 'bold'), wraplength=300, justify='left').pack(anchor='w', padx=10, pady=(0, 8))

        def add_stat(parent, label, value, row, col):
            box = tk.Frame(parent, bg='#f8fafc', bd=1, relief='solid')
            box.grid(row=row, column=col, sticky='nsew', padx=5, pady=5)
            tk.Label(box, text=label, bg='#f8fafc', fg=SUBTEXT, font=('Segoe UI', 8)).pack(anchor='w', padx=10, pady=(8, 0))
            tk.Label(box, text=value, bg='#f8fafc', fg=TEXT, font=('Segoe UI', 10, 'bold'), wraplength=300, justify='left').pack(anchor='w', padx=10, pady=(0, 8))
        ...
        def show_detail(card):
            ...
            rarity = pretty_title(card.get('rarity'))
            elixir = card.get('elixir') if card.get('elixir') not in (None, '') else 'Unknown'
            arena = pretty_arena(card.get('arena'))
            ...
            stats = [
                ('Hitpoints', card.get('hitpoints') if card.get('hitpoints') not in (None, '') else '—'),
                ('Damage', card.get('damage') if card.get('damage') not in (None, '') else '—'),
                ('Range', card.get('card_range') if card.get('card_range') not in (None, '') else '—'),
                ('Stun duration', card.get('stun_duration') if card.get('stun_duration') not in (None, '') else '—'),
                ('Shield', card.get('shield') if card.get('shield') not in (None, '', '-') else '—'),
                ('Movement speed', card.get('movement_speed') if card.get('movement_speed') not in (None, '', '-') else '—'),
                ('Radius', card.get('radius') if card.get('radius') not in (None, '', '-') else '—'),
            ]

            for index, (label, value) in enumerate(overview):
                add_field(info_grid, label, str(value), index // 2, index % 2)
            for c in range(2):
                info_grid.columnconfigure(c, weight=1)

            detail_desc.configure(text=str(card.get('description') or 'No description available.').strip().replace('“', '"').replace('”', '"'))

            for index, (label, value) in enumerate(stats):
                add_stat(stats_grid, label, str(value), index // 2, index % 2)
            for c in range(2):
        ...
        def render_catalog():
            for widget in catalog_inner.winfo_children():
                widget.destroy()
            ...
            for index, card in enumerate(cards):
                tile = self.card_tile(catalog_inner, card, on_click=show_detail, compact=True)
                tile.grid(row=index // columns, column=index % columns, padx=8, pady=8, sticky='nsew')

            for c in range(columns):
                catalog_inner.columnconfigure(c, weight=1)
